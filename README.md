...
| 维度 | 说明 |
|------|------|
| 创建维修记录 | schema 校验 `auth.role` 须包含 `app_maintainer` |
| 创建报修单 | 任何登录用户均可 |
| 首页快捷操作 | 四个按钮始终可见，点击时校验角色：维修用户点"我的报修"提示不可用，报修用户点"维修记录"提示不可用 |
| 列表页 | `list.vue` 双 Tab（我的报修 + 维修记录），数据按当前用户过滤 |
...
首页显示四个快捷操作按钮，所有用户可见，点击时根据角色控制权限：

| 快捷操作 | 跳转页面 | 报修用户 | 维修用户 |
|----------|---------|:---:|:---:|
| 设备列表 | `pages/device/device-list` | ✅ | ✅ |
| 我的报修 | `pages/repair-request/repair` | ✅ | ❌ 提示"仅报修用户可用" |
| 维修记录 | `pages/repair-request/list?tab=1` | ❌ 提示"仅维修用户可用" | ✅ |
| 保养提醒 | `pages/alert/list` | ✅ | ✅ |

### 申请报修

1. 报修用户点击首页 **"我的报修"** → 进入 `repair.vue` 的 **"申请报修"** Tab
...
- 在 `repair.vue` **"报修记录"** Tab 或 `list.vue` **"我的报修"** Tab 中查看自己的报修单
- 每个报修单展示设备名称、故障描述、紧急程度、处理状态标签
- 点击卡片进入**报修详情页**（`detail.vue`）
- **编辑**：仅限未关联维修记录的报修单
- **删除**：存在关联维修记录时禁止删除，同时恢复设备状态为使用中

### 维修记录

- 维修用户通过首页 **"维修记录"** 进入 `list.vue`，Tab 切换查看"我的报修"和"维修记录"
- 显示与当前用户报修单关联的维修记录
